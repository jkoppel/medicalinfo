...
// A..... Create a default progress bar in Status Bar with "Work in progress..." as text
  ProgressInStatusBar(_T("Work in progress"));
}
...
// B..... Create a CStatic object upon the target status bar pane
    ...
    bSuccess   = CStatic::CreateEx(nStyleEx, poStrClass, _T(""), nStyle, oRectPane, poStatusBar, (UINT) this); // Use object's unique address as child window identifier
...
// CA.... Get the length of the cSPT_PERCENT message
            ...
            if(m_nProgressText > 0)
            {
              oStrPercent = _T("(") + oStrPercent + _T(")");
            }else{}

            oStrPercent = _T(" ") + oStrPercent;
...
// CC.... Get the length of the text to display
          ...
          nLenMargin  = oDC.GetTextExtent(_T(" ")).cx << 1; // Text margin
...
#endif // dCSP_SLIDERBAR_METHOD
  oStrTempo.Format(_T("%d%% / "), nTempo >> 8); // '>> 8' == '/ 256'
  *o_poTimed += oStrTempo;

// F..... Add the remaining progress completion
  // Remaining percent
  oStrTempo.Format(_T("%d%% - "), 100 - (nTempo >> 8));
  *o_poTimed += oStrTempo;
...
// G..... Get the remaining time before end
  ...
  // Time to end
  oStrTempo.Format(_T("%02d:%02d:%02d - "), sTempoTimeSystem.wHour, sTempoTimeSystem.wMinute, sTempoTimeSystem.wSecond);
  *o_poTimed += oStrTempo;
