...
/**
  ...
  * 返回 : 校验后的值
  */
ushort CRC16(const uchar *pdata,int len)
{
	ushort	j,crc,uc,tmp;
...
///CRC校验,对输入的十六进制串进行CRC校验,返回相应的两个字节的税控码
/**
  * input : 待校验的十六进制串 \n
  * dlen : 十六进制串的长度,注意外部缓冲区至少为dlen+2 \n
  * crc : 返回两个字节的CRC校验码 \n
  */
void CRC_16(uchar *input,uchar dlen, uchar *crc)
{
	uchar i='\0',j='\0';
	unsigned long a=0u,b=0u;
	if((input==NULL) || (crc==NULL)){
		return;
	}
	*(input+dlen)  =0x00;
	*(input+dlen+1)=0x00;		//初值置0
	a=*input;
	a<<=8;
	input++;
	a |= *input;
	...
	for(j=0;j<dlen;j++)
	{
		...
		{
			a<<=1;
			if((a&0x01000000)==0x01000000){a ^= b;}
		}
	}
	crc[0]=(uchar)(a>>16);
	crc[1]=(uchar)((a>>8)&0xff);
}
