...
CLBTabCtrl::CLBTabCtrl():m_nPrevPointedItem(-1),m_bOverTab(false),
						...
						m_bAutoRaising(false),m_bHasFocus(true)
{
}
...
void CLBTabCtrl::PreSubclassWindow() 
{
	...
	while(pWnd)
	{
		...
		if(nRet && _tcscmp(buf,_T("msctls_updown32")))
		{
...
void CLBTabCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	//use the new font
	CFont font;
	font.CreateFont(16,8,0,0,0,FALSE,FALSE,FALSE,
						DEFAULT_CHARSET,OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,DEFAULT_QUALITY,
						FIXED_PITCH | FF_ROMAN, _T("System"));

	dc.SelectObject(&font);

	//dc.SelectObject(m_hMenuFont);//the orginal value
	TCHAR buf[255];
	...
	tcitem.pszText =buf;
	tcitem.cchTextMax=sizeof(buf)/sizeof(TCHAR);
