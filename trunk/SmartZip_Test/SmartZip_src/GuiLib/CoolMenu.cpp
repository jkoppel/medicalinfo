...
BOOL CCoolMenuManager::LoadToolbar(UINT nIDToolbar)
{
	...
	// First toolbar: initialized bitmap/button sizes and create image list.
		...
		VERIFY(m_ilButtons.Create(sz.cx, sz.cy, ILC_COLORDDB|ILC_MASK, 0, 10));
	...
	m_ilButtons.Add(&bmToolbar,GuiDrawLayer::GetRGBColorFace());
...
BOOL CCoolMenuManager::OnMeasureItem(LPMEASUREITEMSTRUCT lpms)
{
	...
	if (pmd->fType & MFT_SEPARATOR) {
		...
		int cx = rcText.Width()+20;	// text width 
		...
		lpms->itemWidth = cx+14;		// done deal
		lpms->itemHeight+=2;
...
void CCoolMenuManager::DrawSeparator(CRect& rc, CDC& dc)
{
	CBrush brush;
	CRect m_rc=rc;
	brush.CreateSolidBrush(GuiDrawLayer::GetRGBColorXP());
	m_rc.right=m_rc.left+m_szButton.cx+11;
	dc.FillRect(m_rc,&brush);
	brush.DeleteObject( );
	if (m_StyleDisplay==GUISTYLE_2003)
	{
		CGradient M(CSize(m_rc.Width(),m_rc.Height()));	
		M.PrepareHorizontal(&dc,m_StyleDisplay);
		M.Draw(&dc,-1,m_rc.top,0,0,m_rc.Width(),m_rc.Height(),SRCCOPY);	
	}
	brush.CreateSolidBrush(GuiDrawLayer::GetRGBSkinMenu());
	rc.left+=m_szButton.cx+4;
...
BOOL CCoolMenuManager::OnDrawItem(LPDRAWITEMSTRUCT lpds)
{
	...
	if (pmd->fType & MFT_SEPARATOR) {
		...
		if (true) {
			...
			sz = rcButn.Size() - m_szBitmap;
			sz.cx >>= 1;
			sz.cy >>= 1;
			p=CPoint(rcButn.TopLeft() + sz);
			p.x+=2;
			...
			if (!bDisabled) {
				...
				if (bSelected || bChecked) {
					...
					{
						rc2.DeflateRect(5,5);
						GuiDrawLayer::DrawCheck(&dc,rc2);
					}
					//dc.DrawEdge(rc2, bChecked ? GuiDrawLayer::GetRGBCaptionXP() : GuiDrawLayer::GetRGBCaptionXP(),
					//	BF_RECT);
				}
				// draw the button!
				if (bSelected)
				{
					...
					m_ilButtons.GetImageInfo(iButton,&pImageInfo);
					rcima =pImageInfo.rcImage;
					siImg=CSize(rcima.Width(),rcima.Height());
					if (GuiDrawLayer::m_Style != GUISTYLE_2003)
					{
						p.x+=1;
						p.y+=3;
						dc.DrawState(p,siImg,m_ilButtons.ExtractIcon(iButton),DSS_MONO,CBrush (GuiDrawLayer::GetRGBColorShadow()));
						p.x-=2;					p.y-=3;
					}
			...
			GetMenuItemInfo((HMENU)lpds->hwndItem,
				lpds->itemID, MF_BYCOMMAND, (MENUITEMINFO *)&info);
		...
		COLORREF colorText = GetSysColor(bDisabled ?  COLOR_GRAYTEXT :
			bSelected ? COLOR_MENUTEXT : COLOR_MENUTEXT);
...
void CCoolMenuManager::PLSelectRect(CDC& dc, const CRect& rc)
{
	CBrush brush;
	if(m_StyleDisplay==GUISTYLE_2003)
		brush.CreateSolidBrush(GuiDrawLayer::m_Theme ? RGB(252,230,186):GuiDrawLayer::GetRGBFondoXP());
	else
	...
	if(m_StyleDisplay==GUISTYLE_2003)
	{
		...
		if(clrNear==13221564)
		{
			dc.Draw3dRect(rc,GuiDrawLayer::GetRGBColorShadow(GUISTYLE_2003),GuiDrawLayer::GetRGBColorShadow(GUISTYLE_2003)); 
			return;
		}
		
	}
	dc.Draw3dRect(rc,GuiDrawLayer::GetRGBCaptionXP(),GuiDrawLayer::GetRGBCaptionXP()); 

}

void CCoolMenuManager::PLNormalRect(CDC& dc, const CRect& rc)
{
	CRect m_rc=rc;
	CBrush brush;
	brush.CreateSolidBrush(GuiDrawLayer::GetRGBColorXP());
	m_rc.right=m_rc.left+m_szButton.cx+4;
	dc.FillRect(m_rc,&brush);
	brush.DeleteObject( );
	if (m_StyleDisplay==GUISTYLE_2003)
	{
		CGradient M(CSize(m_rc.Width(),m_rc.Height()));	
		M.PrepareHorizontal(&dc,m_StyleDisplay);
		M.Draw(&dc,0,m_rc.top,0,0,m_rc.Width(),m_rc.Height(),SRCCOPY);	
	}	
	m_rc=rc;
	m_rc.left+=m_szButton.cx+4;
	brush.CreateSolidBrush(GuiDrawLayer::GetRGBSkinMenu());
	dc.FillRect(m_rc,&brush);
