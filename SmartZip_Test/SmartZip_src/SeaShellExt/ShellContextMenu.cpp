...
CShellContextMenu::~CShellContextMenu()
{
    ...
    if (m_pSendToMenu)
    {
		...
		pData = CUICoolMenuManager::GetItemData(*m_pSendToMenu, i);
        while (pData)
        {
        ...
        if (IDM_SENDTOFIRST <= iCmd  &&  iCmd <= IDM_SENDTOLAST)
        {
            ...
            if (SUCCEEDED(hr))
            {
                pidlDrop.Set((LPITEMIDLIST)
                    CUICoolMenuManager::GetItemData(*m_pSendToMenu, iCmd));
...
// NOTE: Slight change to return here but change it back if you want: PO
    ...
    {
		//CMenuItemInfo mii;
		MENUITEMINFO mii;//modified by hwy
		...
		if (!sCommand.IsEmpty())
		{
			for(UINT idm=0;idm < pMenu->GetMenuItemCount();idm++)
			{
				...
				if (sMenuText == sDisplayName)
				{
					...
					CUICoolMenuManager::SetItemIcon(*pMenu, hIconApp, idm, TRUE);
					break;
				}
			}
		}
        ...
        if (SUCCEEDED(hr)) 
        {
            ...
            if (ulAttrs & SFGAO_FOLDER) // folder?
            {
                ...
                g_CoolMenuManager.ConvertMenuItem(pMenu, 
                    idx_folder);
                CUICoolMenuManager::SetItemIcon(*pMenu, 
...
// To keep it familiar the SendTo menu is sorted
	...
	STL_FOR_ITERATOR(vecCMSort,vMenuItems)
	{
		...
		CUICoolMenuManager::SetItemData(*pMenu, 
