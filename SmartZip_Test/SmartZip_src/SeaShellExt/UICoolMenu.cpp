...
//                leak in CUIItemData structure, added a destructor that 
//                calls text.Empty().
...
//                ConvertMenu() which allocates a new CUIItemData for each 
...
//                fixed by modifying the ~CUICoolMenuManager() destructor.
...
CUICoolMenuManager g_CoolMenuManager;
...
BOOL CUICoolMenuManager::bTRACE = FALSE;
...
struct CUIItemData {
    ...
    CUIItemData()       { lpcm=NULL; magicNum = MYITEMID; iButton = -1; hIcon = 0; pContext = NULL; }
    ~CUIItemData()        { text.Empty(); if (hIcon) ::DestroyIcon(hIcon); if (lpcm) lpcm->Release(); }
    BOOL IsMyItemData()   { return this != NULL  &&  magicNum == MYITEMID; }
};

static CUIItemData *GetMyItemData(HMENU hMenu, UINT uItem, 
                                  BOOL bByPosition = FALSE)
{
    ...
    return (CUIItemData *)mii.dwItemData;
}

static void SetMyItemData(CUIItemData *pmd, HMENU hMenu, UINT uItem, 
                          BOOL bByPosition = FALSE)
{
    MENUITEMINFO mii;
    mii.cbSize     = sizeof(mii);
...
IMPLEMENT_DYNAMIC(CUICoolMenuManager, CSubclassWnd)

CUICoolMenuManager::CUICoolMenuManager()
{
	m_lpcm = NULL;		
...
//
void CUICoolMenuManager::Destroy()
{
...
//
void CUICoolMenuManager::DestroyAccel()
{
...
//
void CUICoolMenuManager::Install(CWnd* pFrame)
{
	m_pFrame = pFrame;
	HookWindow(pFrame);   // install message hook
}

void CUICoolMenuManager::Uninstall()
{
	m_pFrame = NULL;
...
void CUICoolMenuManager::SetShellContextMenu(LPCONTEXTMENU lpcm,UINT nIDFirst,UINT nIDLast)
{
	m_lpcm = lpcm;
...
//
BOOL CUICoolMenuManager::LoadToolbars(const UINT* arID, int n)
{
  ASSERT(arID);
  BOOL bRet = TRUE;
  for (int i=0; i<n; i++)
...
CSize CUICoolMenuManager::SetButtonSize(CSize &sz)
{
	// Set defaults
	if (sz.cx == 0)
		sz.cx = 16;
...
//
BOOL CUICoolMenuManager::LoadToolbar(UINT nIDToolbar)
{
  // load bitmap
  ...
  for (int i = 0; i < ptbd->wItemCount; i++) {
    ...
    if (nID > 0) {
      if (GetButtonIndex(nID) >= 0) {
        ...
        TRACE(_T("CUICoolMenuManager::LoadToolbar(). Added Menu Id %d, Button Number %d\n"), nID, iNextImage-1);
      }
...
//
LRESULT CUICoolMenuManager::WindowProc(UINT msg, WPARAM wp, LPARAM lp)
{
  switch(msg) {
...
//
void CUICoolMenuManager::Refresh()
{
...
//
CFont* CUICoolMenuManager::GetMenuFont()
{
  if (!(HFONT)m_fontMenu) {
...
//
BOOL CUICoolMenuManager::OnMeasureItem(LPMEASUREITEMSTRUCT lpms)
{
	ASSERT(lpms);
	CUIItemData* pmd = (CUIItemData*)lpms->itemData;
	ASSERT(pmd);
...
//
BOOL CUICoolMenuManager::OnDrawItem(LPDRAWITEMSTRUCT lpds)
{
	ASSERT(lpds);
	...
	// Omar L Francisco
	CUIItemData* pmd = (CUIItemData*)lpds->itemData;
	if (!pmd->IsMyItemData())
		return FALSE;
	// Addition: Philip Oldaker
...
//
void CUICoolMenuManager::DrawMenuText(CDC& dc, CRect rc, CString text,
  COLORREF color)
{
  CString left = text;
  CString right;
...
//
BOOL CUICoolMenuManager::Draw3DCheckmark(CDC& dc, const CRect& rc,
...
//
void CUICoolMenuManager::OnInitMenuPopup(CMenu* pMenu,
  UINT nIndex, BOOL bSysMenu)
{
	...
	{
		CUIItemData* pmd = GetMyItemData(*pMenu,i,TRUE);
		if (pmd->IsMyItemData() && pmd->lpcm)
		{
			...
			if (SUCCEEDED(hr))
			{
				SetMyItemData((CUIItemData*)pmd->pContext,*pMenu,i,TRUE); 
...
//
void CUICoolMenuManager::LoadAccel(HACCEL hAccel)
{
  DestroyAccel();
  int nAccel;
...
void CUICoolMenuManager::ConvertMenu(CMenu *pMenu)
{
    ConvertMenu(pMenu, 0, FALSE, TRUE);
}

void CUICoolMenuManager::UnconvertMenu(CMenu *pMenu)
{
    ConvertMenu(pMenu, 0, FALSE, FALSE);
}

void CUICoolMenuManager::ConvertMenuItem(CMenu *pMenu, int iIndex, 
    bool bRecurse /*= true*/, bool bUnconvert /*= false*/)
{
    ...
    CUIItemData* pmd = (CUIItemData*)info.dwItemData;
    ...
    if ((info.fType & MFT_OWNERDRAW)  && pmd  &&  !pmd->IsMyItemData()) {
        CMTRACE(_T("CUICoolMenuManager: ignoring foreign owner-draw item\n"));
        return;
...
/*    // Koji MATSUNAMI 1999.2.23
    ...
    if (bSysMenu && info.wID >= 0xF000) {
        CMTRACE(_T("CUICoolMenuManager: ignoring sys menu item\n"));
    ...
    if (!bUnconvert) 
    {
        ...
        if (!(info.fType & MFT_OWNERDRAW))
		{
			...
			{                   // if no item data:
				...
				{
					CUIItemData* pnmd = new CUIItemData;    //   create one
					pnmd->pContext = pmd;
					pmd = pnmd;
				}
				else
					pmd = new CUIItemData;    //   create one
                ASSERT(pmd);              //   (I hope)
...
///////////////////////
// Addition: Philip Oldaker
DWORD CUICoolMenuManager::SwitchContextItemData(CMenu *pMenu, int iCmd, DWORD dwItemData /*0*/, BOOL bByPosition /*TRUE*/)
{
	CUIItemData* pmd = GetMyItemData(*pMenu,iCmd,bByPosition);
	DWORD dwData=0;	
	if (pmd == NULL)
		return dwData;
	CMyMenuItemInfo info;
	...
	{
		CUIItemData *pMyItemData = (CUIItemData*)dwItemData;
...
void CUICoolMenuManager::AddShellContextMenu(CMenu *pMenu, LPCONTEXTMENU lpm, int iIndex)
{
	CUIItemData* pmd = GetMyItemData(*pMenu,iIndex,TRUE);
...
//
void CUICoolMenuManager::ConvertMenu(CMenu* pMenu, UINT nIndex, 
...
void *CUICoolMenuManager::SetItemData(HMENU hMenu, void *pData, UINT uItem, BOOL bByPosition /*= FALSE*/)
{
    CUIItemData *pmd = GetMyItemData(hMenu, uItem, bByPosition);
...
void *CUICoolMenuManager::GetItemData(HMENU hMenu, UINT uItem, BOOL bByPosition /*= FALSE*/)
{
    MENUITEMINFO mii;
    mii.cbSize     = sizeof(mii);
...
if (mii.dwItemData == -1) _asm int 3;    
    CUIItemData *pmd = (CUIItemData *)mii.dwItemData;
...
HICON CUICoolMenuManager::SetItemIcon(HMENU hMenu, HICON hIcon, UINT uItem, BOOL bByPosition /*= FALSE*/)
{
    CUIItemData *pmd = GetMyItemData(hMenu, uItem, bByPosition);
    ASSERT(pmd->IsMyItemData());
...
HICON CUICoolMenuManager::GetItemIcon(HMENU hMenu, UINT uItem, BOOL bByPosition /*= FALSE*/)
{
    CUIItemData *pmd = GetMyItemData(hMenu, uItem, bByPosition);
    ASSERT(pmd->IsMyItemData());
    return pmd->hIcon;
...
void CUICoolMenuManager::SetSubMenuIcon(HMENU hSubMenu, int nID)
{
    m_mapHMENUtoID[(void *)hSubMenu] = nID;
}

void CUICoolMenuManager::UnmanageSubMenu(HMENU hSubMenu)
{
...
//
LRESULT CUICoolMenuManager::OnMenuChar(UINT nChar, UINT nFlags, CMenu* pMenu)
{
	ASSERT_VALID(pMenu);
	CUIItemData* pmd = GetMyItemData(*pMenu,0,TRUE);
	// Addition: Philip Oldaker
	// Pass it on to the shell if this has been set
	if (pmd->IsMyItemData() && pmd->lpcm)
	{
...
//
void CUICoolMenuManager::OnMenuSelect(UINT nItemID, UINT nFlags, HMENU hSysMenu)
{
	if (m_lpcm)
	{
...
CString CUICoolMenuManager::GetVirtualKeyName( const CString strVirtKey ) const
{
  CString strResult;
...
//
BOOL CUICoolMenuManager::AppendAccelName(CString& sItemName, UINT nID)
{
...
//
void CUICoolMenuManager::FixMFCDotBitmap()
{
...
//
HBITMAP CUICoolMenuManager::GetMFCDotBitmap()
{
...
// Peter Tewkesbury
BOOL CUICoolMenuManager::AddSingleBitmap(UINT nBitmapID, UINT n, UINT *nID)
{
  // load bitmap
