...
int CMimeEncodedWord::Decode(unsigned char* pbOutput, int nMaxSize)
{
	...
	while (pbData < pbEnd)
	{
		...
		if ( nMaxSize > 2 && pbData[0] == '=' && pbData[1] == '?')	// it might be an encoded-word
		{
			pszHeaderEnd = FindChar(pbData+2, '?', pbEnd );
			if (pszHeaderEnd != NULL && pszHeaderEnd[2] == '?' && pszHeaderEnd+3 < pbEnd)
			{
				nCoding = tolower(pszHeaderEnd[1]);
				pszHeaderEnd += 3;
				pszCodeEnd = FindString(pszHeaderEnd, "?=", pbEnd );
		...
		{
			pszCodeEnd = FindString(pbData+1, "=?", pbEnd );
			if (!pszCodeEnd || pszCodeEnd >= pbEnd)
				pszCodeEnd = pbEnd;
