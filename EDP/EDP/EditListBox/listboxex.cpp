                              ...
                              LPCTSTR szItemText )
{
   ...
   dwItemData = (DWORD)GetItemData( iItem );//modified by hwy
                            ...
                            int iSecondItem )
{
   ...
   if ( iFirstItem != iSecondItem )
   {
	  ...
	  dwFirstItemData = (DWORD)GetItemData( iFirstItem );//modified by hwy
	  ...
	  dwSecondItemData = (DWORD)GetItemData( iSecondItem );//modified by hwy
...
CListBoxExBuddy::CListBoxExBuddy()
{
   ...
   TCHAR *szFileName = _tempnam( NULL, NULL );//modified by hwy
   FILE *pfBmpFile = NULL;
   fopen_s(&pfBmpFile, szFileName, "wb" );
...
int CListBoxExBuddy::FindButton( const CPoint & point )
{
   UINT iIndex;
   // Find the button
   for (iIndex = 0; iIndex < __BMP_NUMBTN; iIndex++ )
   {
                                  ...
                                  LPTSTR szTipText )
{
   ...
   if ( nID < __BMP_NUMBTN )
   {
      _tcscpy_s( szTipText, 20, aszTips[ nID ] );
   }
                                ...
                                LRESULT *pResult )
{
   ...
   if ( nCode == TTN_GETDISPINFO )
   {
      ...
      UINT nID = (UINT)((NMHDR *)lParam)->idFrom - 1;//modified by hwy
      SetTipText( nID, 
