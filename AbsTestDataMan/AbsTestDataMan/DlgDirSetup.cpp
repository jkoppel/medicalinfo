...
BOOL CDlgDirSetup::OnInitDialog()
{
	...
	for(int i=0;i<cMgt.m_saDirectories.GetCount();i++){
		CString str = cMgt.m_saDirectories.GetAt(i);
...
void CDlgDirSetup::OnBnClickedOk()
{
	...
	cMgt.m_saDirectories.RemoveAll();
	for(int i=0;i<m_ctrlListDir.GetCount();i++){
		m_ctrlListDir.GetText(i, dir);
		str = CString(dir);
		str = str.Trim();
		if(str.GetLength()){
			cMgt.m_saDirectories.Add(str);
		}
	}
	cMgt.SaveDirToConfigFile();
