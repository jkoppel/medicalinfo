...
// pszEnd points to after the last valid character to test in str1
const char* FindString(const char* pszStr1, const char* pszStr2, const char* pszEnd)
{
	// set to last valid character to start test in str1
...
// search for character c in string (strchr)
...
string CMimeHeader::GetMainType() const
{
	...
	if (pszType != NULL)
	{
		const char* pszSlash = ::strchr(pszType, '/'); // strchr safe because GetContentType now returns null-terminated string
		if (pszSlash != NULL)
...
int CMimeHeader::Load(const char* pszData, int nDataSize)
{
	...
	return (pszData[nInput] == '\r') ? nInput + 2 : nInput; // skip the ending CRLF
}
...
bool CMimeBody::ReadFromFile(const char* pszFilename)
{
	...
	try
	{
		...
		::fseek(hFile, 0L, SEEK_END); // get file length
		fpos_t pos;
		...
		if (nFileSize > 0)
		{
			AllocateBuffer(nFileSize+1); // room for terminator
...
void CMimeMessage::SetDate(int nYear, int nMonth, int nDay, int nHour, int nMinute, int nSecond)
{
	...
	::localtime_s(&tmDate, &timeDate);			// adjusted local time
	struct tm tmGmt;
	::gmtime_s(&tmGmt, &timeDate);	// Greenwich Mean Time
